...
func startHTTPServer(ipAddress string, registerHandlers bool, swaggerFile string) common.SyncServiceError {
	...
	if common.ServingAPIs || (common.Configuration.NodeType == common.CSS &&
		...
		if common.Configuration.ListeningType == common.ListeningUnsecurely {
			...
			log.Info("chmod to 0777 for socket file")
			...
			if err != nil {
				return &common.SetupError{Message: fmt.Sprintf("Failed to check permission for Unix Socket listening. Error: %s", err)}
			}
			mode := info.Mode()
			log.Info("Group permission: ")
			for i := 4; i < 7; i++ {
				log.Info(string(mode.String()[i]))
			}

			log.Info("Others permission: ")
			for i := 7; i < 10; i++ {
				log.Info(string(mode.String()[i]))
			}
